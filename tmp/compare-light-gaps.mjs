...
// Original paragraph structure from user paste (blank line = paragraph break)
...
function blocks(text) {
	// Split into structural units: heading or paragraph
	// In original: single newlines between paragraphs under a section = consecutive paras
	// blank line = larger gap
	...
	for (const line of lines) {
		...
		const gap = blankRun; // 0 = adjacent lines, 1+ = blank lines between
		blankRun = 0;
		const clean = line.replace(/^[\u3000\s]+/, '');
		const kind = clean.startsWith('■') ? 'h' : 'p';
		units.push({ kind, gap, preview: clean.slice(0, 40) });
	}
	return units;
}

function stripCurrent(mdx) {
...
const origUnits = blocks(original);
const currUnits = blocks(stripCurrent(fs.readFileSync('src/content/articles/vol3/light.mdx', 'utf8')));

console.log('orig units', origUnits.length, 'curr units', currUnits.length);

const n = Math.max(origUnits.length, currUnits.length);
for (let i = 0; i < n; i++) {
	const o = origUnits[i];
	const c = currUnits[i];
	if (!o) {
		console.log(`EXTRA CURR[${i}] gap=${c.gap} ${c.kind} ${c.preview}`);
		continue;
	}
	if (!c) {
		console.log(`MISSING CURR[${i}] gap=${o.gap} ${o.kind} ${o.preview}`);
		continue;
	}
	const gapDiff = o.gap !== c.gap;
	const kindDiff = o.kind !== c.kind;
	const textDiff = o.preview !== c.preview && !c.preview.startsWith(o.preview.slice(0, 20));
	if (gapDiff || kindDiff || textDiff) {
		console.log(`DIFF[${i}]`);
		console.log(`  orig gap=${o.gap} ${o.kind} ${o.preview}`);
		console.log(`  curr gap=${c.gap} ${c.kind} ${c.preview}`);
	}
}

console.log('\n--- Original structure (gap before each unit) ---');
origUnits.forEach((u, i) => {
	const mark = u.gap >= 1 ? `[blank×${u.gap}]` : '[tight]';
	console.log(`${String(i).padStart(2)} ${mark} ${u.kind} ${u.preview}`);
});
